...
from utils.utils_ import get_num_classes, show_config
...
if __name__ == "__main__":
    ...
    #--------------------------------------------------------#
    annotation_path = "/mnt/my_face/cls_train.txt"
    #--------------------------------------------------------#
    ...
    #----------------------------------------------------------------------------------------------------------------------------#  
    model_path = "/mnt/my_face/facenet_mobilenet.pth"
    #----------------------------------------------------------------------------------------------------------------------------#
    ...
    # ----------------------------------------------------------------------------------------------------------------------------# 显存不足与数据集大小无关，提示显存不足请调小batch_size。 受到BatchNorm层影响，不能为1。
    #
    ...
    #------------------------------------------------------------------#
    lfw_dir_path = "/mnt/my_face/lfw"
    lfw_pairs_path = "/mnt/my_face/lfw_pair.txt"
    ...
    if model_path != '':
        ...
        model_dict.update(temp_dict)  #将预训练权重中的键值对添加到模型的状态字典中。
        model.load_state_dict(model_dict)  #将预训练权重加载到模型中
        #------------------------------------------------------#
        #   显示没有匹配上的Key
        #------------------------------------------------------#
        if local_rank == 0:
